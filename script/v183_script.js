// v183_script.js — extracted from 183_accepted_fifa_world_cup_2026.html
        ...
        // v172: Expose selectedYear, g, map, and the highlighted team to window
        // so the right-click popup IIFE (in a separate <script> block) can access them.
        window.__getSelectedYear = () => selectedYear;
        window.__getD3Group = () => typeof g !== 'undefined' ? g : null;
        window.__getMap = () => typeof map !== 'undefined' ? map : null;
        // v170: These are overridden by the popup IIFE when a team is highlighted.
        // draw() checks them to apply highlight styles during rendering.
        window.__getHighlightedTeam = () => null;
        window.__getHighlightedTeamColor = () => null;

        // v172: Helper — resolve team name through alias mapping.
        // Returns a canonical name so "Turkey" and "Türkiye" match.
        // Used in draw() for highlight matching and in buildVenueLinks for participant lookup.
        ...
        // v178: Resolve host nation names from stadiumFullDetails.country field.
        // Handles short-form (MEX, USA, CAN) and alternate names (Korea/Japan, West Germany).
        function resolveHostName(name) {
            ...
            const map = {
                ...
                'KOR': 'South Korea', 'South Korea': 'South Korea', 'Korea Republic': 'South Korea',
                'JPN': 'Japan', 'Japan': 'Japan',
                'GER': 'Germany', 'Germany': 'Germany', 'West Germany': 'Germany', 'FRG': 'Germany',
        ...
        // v174: REMOVED "Round 16" and "Group" from specialColors. These should use
        // the vibrantColor (year color hue shift) as their node color — that's the
        // intended behavior. Only top finishers (Champion, Final, Semi, QF) and hosts
        // get fixed special colors. Group/Round 16/Round 32 teams use the year color.
        ...
        function processData() {
            ...
            byYear.forEach((participants, year) => {
                ...
                // v165: Forked host-line rendering.
                // OLD: each host → each participant = H × N lines (e.g. 3 hosts × 48 teams = 144 lines).
                // NEW: each participant → host hub (centroid) + hub → each host = N + H lines.
                // For 2026: 48 + 3 = 51 lines (down from 144). For 2030: 48 + 6 = 54 (down from 288).
                // The hub is the geographic centroid of all hosts for that year.
                // Lines are drawn as: participant → hub (single line per participant),
                // then hub → each host (short fan). This maintains the visual "connection
                // to host" while eliminating the combinatorial explosion.
                if (hosts.length > 0) {
                    // Compute host centroid (average lat/lng of all hosts)
                    const hostCentroid = {
                        lat: hosts.reduce((sum, h) => sum + h.lat, 0) / hosts.length,
                        lng: hosts.reduce((sum, h) => sum + h.lng, 0) / hosts.length,
                        year: year
                    };
                    // Store the hub as a virtual node (not rendered, just for line endpoints)
                    // We tag links with the year so filterAndDraw can group them.
                    participants.forEach(p => {
                        if (p.Success !== "Host" && p.id !== hosts[0].id) {
                            // Participant → hub link (the "trunk" of the fork)
                            allLinks.push({
                                source: hostCentroid,  // starts at hub
                                target: p,              // ends at participant
                                year: year,
                                color: vibrantColor,
                                linkType: 'hub-to-participant'  // v165: tag for rendering
                            });
                        }
                    });
                    // Hub → each host (the "fan" — short lines from centroid to each host)
                    hosts.forEach(host => {
                        allLinks.push({
                            source: hostCentroid,
                            target: host,
                            year: year,
                            color: vibrantColor,
                            linkType: 'hub-to-host'  // v165: tag for rendering
                        });
                    });
                }
            });
        ...
        function resetFilter() {
            ...
            updateFisheyeUI();
            filterAndDraw();
            // v174: Close the participant popup when year filter is reset.
            // The popup is year-dependent — it only makes sense when a year is selected.
            if (window.__participantPopup && typeof window.__participantPopup.close === 'function') {
                window.__participantPopup.close();
            }
            // Reset Historical
        ...
        async function handleYearChange(year) {
            selectedYear = year;
            updateFisheyeUI();
            // v176: If the participant popup is open, reload it with the new year's data
            // instead of closing it. The popup stays open, the participant list updates,
            // and the highlight color changes to match the new year's success status.
            if (window.__participantPopup && typeof window.__participantPopup.reload === 'function') {
                window.__participantPopup.reload(year);
            }
            filterAndDraw(); // Core D3 — initial render (links may be empty if data not loaded yet)

            if (year >= 1930) {
                try { await loadHistoricalData(year); } catch (e) { void 0; }
                // v173: Re-render after data loads. buildVenueLinks needs FWCD_{year}.matches_FULL
                // which is loaded asynchronously. Without this second filterAndDraw(), the venue
                // links don't appear until the user manually zooms (which triggers filterAndDraw
                // via the zoomend event). This was the "lines not loading until zoom" bug.
                filterAndDraw();
                // v176: Reload popup AFTER data loads so getNodeColor can find the new year's
                // nodes (which are built during processData/filterAndDraw). The first reload
                // (before await) updates the participant list; this second reload refreshes
                // the colors now that the D3 nodes exist for the new year.
                if (window.__participantPopup && typeof window.__participantPopup.reload === 'function') {
                    window.__participantPopup.reload(year);
                }
        ...
        function generateDynamicElements(nodes, links) {
            ...
            for (let i = -copiesNeeded; i <= copiesNeeded; i++) {
                ...
                // v171: Spread ALL link properties (teamName, linkType, color, venueName)
                // so draw() can check d.teamName for highlighting. Previously only source
                // and target were copied, losing teamName/linkType — so highlight never worked.
                links.forEach(l => renderLinks.push({
                    ...l,  // copy ALL properties: teamName, linkType, color, venueName, year
        ...
        function draw() {
            ...
            // v170: Check if a team is highlighted via the popup filter.
            // If so, apply highlight styles during rendering — this is the ONLY
            // reliable way to keep the highlight across map move/zoom, because
            // D3's .join() recreates all elements on every draw() call.
            const hlTeam = window.__getHighlightedTeam ? window.__getHighlightedTeam() : null;
            const hlColor = window.__getHighlightedTeamColor ? window.__getHighlightedTeamColor() : '#FFD700';

            // v172: Render venue fork/web links.
            // DEFAULT mode: all lines are white/grey (NOT vibrant year color).
            // FILTER mode (hlTeam set): highlighted team's lines use success color,
            // 2.5px stroke, full opacity, with glow. Other lines stay visible but
            // dimmed (0.2 opacity) — NOT disabled.
            g.selectAll('.host-line').data(renderLinks).join('path').attr('class', 'host-line')
                ...
                .attr('stroke', d => {
                    // v177: Highlighted team's lines use success color.
                    // Non-highlighted lines use white/grey (#ccc) — NOT the year color.
                    if (hlTeam && (d.teamName === hlTeam || resolveAlias(d.teamName) === resolveAlias(hlTeam))) return hlColor;
                    return '#ccc';  // v172: default white/grey
                })
                .attr('stroke-width', d => {
                    if (hlTeam && (d.teamName === hlTeam || resolveAlias(d.teamName) === resolveAlias(hlTeam))) return 2.5;  // v172: halved from 5.0
                    if (hlTeam) {  // dim mode: keep visible but dimmed
                        if (d.linkType === 'hub-to-host') return 1.5;
                        return 1.0;
                    }
                    // normal mode
                    if (d.linkType === 'hub-to-host') return selectedYear ? 2.0 : 1.2;
                    if (d.linkType === 'fork-to-venue') return selectedYear ? 1.5 : 1.0;
                    if (d.linkType === 'team-to-fork') return selectedYear ? 1.2 : 0.8;
                    return selectedYear ? 1.0 : 0.7;
                })
                .attr('stroke-opacity', d => {
                    if (hlTeam && (d.teamName === hlTeam || resolveAlias(d.teamName) === resolveAlias(hlTeam))) return 1.0;  // highlighted: full
                    if (hlTeam) {  // dim mode — visible but dimmed (NOT disabled)
                        if (d.linkType === 'hub-to-host') return 0.3;
                        return 0.2;  // v172: raised from 0.03 — keep visible
                    }
                    // normal mode
                    if (d.linkType === 'hub-to-host') return selectedYear ? 1.0 : 0.8;
                    if (d.linkType === 'fork-to-venue') return selectedYear ? 0.85 : 0.6;
                    if (d.linkType === 'team-to-fork') return selectedYear ? 0.7 : 0.5;
                    return selectedYear ? 0.6 : 0.4;
                })
                .style('filter', d => {
                    if (hlTeam && (d.teamName === hlTeam || resolveAlias(d.teamName) === resolveAlias(hlTeam))) return `drop-shadow(0 0 6px ${hlColor})`;
                    return 'none';
                })
            ...
            g.selectAll('circle').data(renderNodes).join('circle')
                ...
                .attr('r', d => d.radius)  // v170: keep original radius (don't change size)
                .attr('fill', d => d.color)
                .attr('fill-opacity', d => {
                    if (hlTeam && d.name === hlTeam) return 1;
                    return selectedYear ? 0.8 : 0.5;
                })
                .attr('stroke', d => {
                    if (hlTeam && d.name === hlTeam) return hlColor;
                    return '#fff';
                })
                .attr('stroke-width', d => {
                    if (hlTeam && d.name === hlTeam) return 3;
                    return 1.125;
                })
                .style('filter', d => {
                    if (hlTeam && d.name === hlTeam) return `drop-shadow(0 0 12px ${hlColor}) drop-shadow(0 0 6px ${hlColor})`;
                    return 'none';
                })
            ...
            // v171: Draw venue highlight circles when a team is highlighted.
            // The stadium markers are Leaflet markers (not D3), so we can't style
            // them directly. Instead, we draw pulsing circles at each venue location
            // the highlighted team played at, using the fork-to-venue link data.
            if (hlTeam) {
                // Collect unique venue coordinates from fork-to-venue links for the highlighted team
                // v172: Use alias-aware matching so "Turkey" matches links with teamName "Türkiye"
                const hlResolved = resolveAlias(hlTeam);
                const venuePoints = [];
                const seen = new Set();
                renderLinks.forEach(l => {
                    if (l.linkType === 'fork-to-venue' && l.target &&
                        (l.teamName === hlTeam || resolveAlias(l.teamName) === hlResolved)) {
                        const key = `${l.target.lat},${l.target.lng}`;
                        if (!seen.has(key)) {
                            seen.add(key);
                            venuePoints.push({
                                lat: l.target.lat,
                ...
                // Draw highlight circles at each venue
                g.selectAll('.venue-highlight').data(venuePoints).join('circle')
                    .attr('class', 'venue-highlight')
                    .attr('cx', d => map.latLngToLayerPoint([d.lat, d.lng]).x)
                    .attr('cy', d => map.latLngToLayerPoint([d.lat, d.lng]).y)
                    .attr('r', 14)
                    .attr('fill', 'none')
                    ...
                    .style('filter', `drop-shadow(0 0 8px ${hlColor})`)
                    .style('pointer-events', 'none');
            } else {
                // Remove venue highlights when no team is highlighted
                g.selectAll('.venue-highlight').remove();
            }
        }
        ...
        function filterAndDraw() {
            ...
            if (selectedYear !== null) {
                ...
                // v166: When a year is selected, build participant→venue fork/web links
                // instead of the default host→participant links.
                // Each participant team gets lines to the VENUES they played at,
                // forking from a point near the host to reduce visual clutter.
                filteredLinks = buildVenueLinks(selectedYear);
            }
            generateDynamicElements(filteredNodes, filteredLinks);
            draw();
        }

        // v166: Build participant→venue fork/web links for the selected year.
        // For each participant team, finds all venues they played at (from FWCD_{year}
        // match data), then creates forked links: team → fork point (near host) → venue.
        // The fork point is computed as a point offset from the host centroid toward
        // the team, creating a "web" pattern where lines diverge near the host.
        function buildVenueLinks(year) {
            const varName = `FWCD_${year}`;
            const data = window[varName];
            if (!data || !data.matches_FULL) return [];

            // Get hosts for this year (from FWCD_Participants)
            const hosts = (typeof fifaWorldCupParticipants !== 'undefined')
                ? fifaWorldCupParticipants.filter(p => p.year === year && p.Success === 'Host')
                : [];
            if (hosts.length === 0) return [];

            // Host centroid (average of all host coordinates)
            const hostCentroid = {
                lat: hosts.reduce((s, h) => s + h.lat, 0) / hosts.length,
                lng: hosts.reduce((s, h) => s + h.lng, 0) / hosts.length
            };

            // Get all participants for this year (from FWCD_Participants)
            const participants = (typeof fifaWorldCupParticipants !== 'undefined')
                ? fifaWorldCupParticipants.filter(p => p.year === year && p.Success !== 'Host')
                : [];
            // v172: Build alias-aware participant map. Key by BOTH the original name
            // AND the resolved alias, so "Turkey" (from matches) finds "Türkiye" (from participants).
            const participantMap = {};
            participants.forEach(p => {
                participantMap[p.name] = p;
                participantMap[resolveAlias(p.name)] = p;  // also key by canonical alias
            });

            // For each match, record which venues each team played at
            // teamVenues: { teamName → Set of { lat, lng, name } }
            const teamVenues = {};
            data.matches_FULL.forEach(m => {
                const sDetails = m.stadiumFULLDetails || m.stadiumFullDetails;
                if (!sDetails || !sDetails.coords) return;
                const venueLat = parseFloat(sDetails.coords.lat);
                const venueLng = parseFloat(sDetails.coords.long != null ? sDetails.coords.long : sDetails.coords.lng);
                if (isNaN(venueLat) || isNaN(venueLng)) return;
                const venuePoint = { lat: venueLat, lng: venueLng, name: sDetails.name || '' };

                [m.homeTeam, m.awayTeam].forEach(team => {
                    if (!team) return;
                    if (!teamVenues[team]) teamVenues[team] = [];
                    // Deduplicate venues (a team may play multiple matches at the same venue)
                    const exists = teamVenues[team].some(v => v.lat === venueLat && v.lng === venueLng);
                    if (!exists) teamVenues[team].push(venuePoint);
                });
            });

            // Build forked links: team → fork point → each venue
            const links = [];
            const vibrantColor = yearColorMap[year] || '#fff';

            Object.entries(teamVenues).forEach(([teamName, venues]) => {
                // v172: Try both the raw team name AND the alias-resolved name.
                // "Turkey" in matches → resolveAlias("Turkey") = "Turkey" → participantMap["Turkey"]
                // "Türkiye" in participants → resolveAlias("Türkiye") = "Turkey" → participantMap["Turkey"]
                const teamNode = participantMap[teamName] || participantMap[resolveAlias(teamName)];
                if (!teamNode) return;  // team not in participants data (no coords)

                // Compute fork point: a point between the team and the host centroid,
                // but closer to the host (at 85% of the way from team to host).
                // This makes lines diverge near the host, creating a web pattern.
                const forkLat = teamNode.lat + (hostCentroid.lat - teamNode.lat) * 0.85;
                const forkLng = teamNode.lng + (hostCentroid.lng - teamNode.lng) * 0.85;
                const forkPoint = { lat: forkLat, lng: forkLng };

                // Link 1: team → fork point (the "stem" of the fork)
                links.push({
                    source: { lat: teamNode.lat, lng: teamNode.lng },
                    target: forkPoint,
                    year: year,
                    color: vibrantColor,
                    linkType: 'team-to-fork',
                    teamName: teamName
                });

                // Links 2+: fork point → each venue (the "tines" of the fork)
            ...
            // Also draw hub→host fan lines (connecting fork area to host nodes)
            hosts.forEach(host => {
                links.push({
                    source: hostCentroid,
                    target: { lat: host.lat, lng: host.lng },
                    year: year,
                    color: vibrantColor,
                    linkType: 'hub-to-host'
                });
            });
...
window.__navBack = async function() {
    ...
    if (parseInt(prev.year, 10) !== parseInt(histLookups.currentYear, 10)) {
        ...
        if (yearOpt) {
            ...
            updateFisheyeUI();
            filterAndDraw();
            try { await loadHistoricalData(prev.year); } catch (e) { void 0; }
            filterAndDraw();
...
window.__showRefMatch = async function(matchYear, homeTeam, awayTeam, dateStr, context) {
    ...
    if (parseInt(matchYear, 10) !== parseInt(histLookups.currentYear, 10)) {
        ...
        if (yearOpt) {
            ...
            updateFisheyeUI();
            filterAndDraw();
            try { await loadHistoricalData(matchYear); } catch (e) { void 0; }
            filterAndDraw();
...
async function buildLeaderboardData() {
    ...
    const stats = {
        ...
        mostHostedNations: [], // v165: { nation, matches, stadiums }
    ...
    // v178: Most hosted NATIONS — count TOURNAMENTS hosted and VENUES per nation.
    // Uses stadiumFullDetails.country from match data to determine which country
    // each stadium is actually in (NOT tournament_info.host which lists ALL co-hosts).
    // For 2026: stadiums in Mexico → Mexico's count, stadiums in Canada → Canada's count.
    // Also handles short-form names (MEX→Mexico, USA→United States) via alias resolution.
    const nationData = {};  // nation → { years: Set, stadiums: Set of stadium names }
    for (const year of getCrossYearSources()) {
        const varName = `FWCD_${year}`;
        const data = window[varName];
        if (!data || !data.matches_FULL) continue;

        // Get host nations for this year from tournament_info
        let hostNations = [];
        if (data.tournament_info && Array.isArray(data.tournament_info.host)) {
            hostNations = data.tournament_info.host;
        }
        if (hostNations.length === 0) continue;

        // For each match, get the stadium's country from stadiumFullDetails
        const stadiumCountryMap = {};  // stadiumName → country (from stadiumFullDetails)
        data.matches_FULL.forEach(m => {
            const sDetails = m.stadiumFullDetails || m.stadiumFULLDetails;
            if (sDetails && sDetails.name && sDetails.country) {
                stadiumCountryMap[sDetails.name] = sDetails.country;
            }
        });

        // For each unique stadium in this year, attribute it to its actual country
        const seenStadiums = new Set();
        Object.entries(stadiumCountryMap).forEach(([stadiumName, country]) => {
            if (seenStadiums.has(stadiumName)) return;
            seenStadiums.add(stadiumName);

            // Resolve short-form country names to full names
            const resolvedNation = resolveHostName(country);

            // Only count if this country is one of the host nations for this year
            const isHost = hostNations.some(h => resolveHostName(h) === resolvedNation);
            if (!isHost) return;

            if (!nationData[resolvedNation]) {
                nationData[resolvedNation] = { years: new Set(), stadiums: new Set() };
            }
            nationData[resolvedNation].years.add(year);
            nationData[resolvedNation].stadiums.add(stadiumName);
        });
    }
    stats.mostHostedNations = Object.entries(nationData)
        .map(([nation, data]) => ({
            nation,
            tournaments: data.years.size,
            years: Array.from(data.years).sort((a, b) => a - b),
            stadiums: data.stadiums.size
        }))
        .sort((a, b) => b.tournaments - a.tournaments || b.years[b.years.length - 1] - a.years[a.years.length - 1])
        .slice(0, 15);
...
function renderHostedChart(data) {
    // v182: Split chart with FIXED center divider.
    // Stadium names INSIDE the bar. Left bars 340px, right bars 240px.
    // Headers: "🏟️ Most Hosted Stadiums" and "🏳️ Most Hosted Nations".
    // Nation text: [Flag] 3 · (1970, 1986, 2026) · 14 venues — bold numbers.
    // Font size 0.65rem for all text.
    const stadiumsData = data || [];
    const nationsData = (leaderboardData && leaderboardData.mostHostedNations) || [];
    if (!stadiumsData.length && !nationsData.length) {
        return '<p style="color:#888;text-align:center;padding:20px;">No data</p>';
    }

    const tab = currentLeaderboardTab;
    const leftHalf = 340;
    const rightHalf = 240;
    const barH = 30;  // v182: slightly taller for larger font
    const fontSize = '0.65rem';  // v182: increased from 0.55rem

    const maxStadiumMatches = Math.max(...stadiumsData.map(d => d.matches), 1);
    const maxNationTournaments = Math.max(...nationsData.map(d => d.tournaments), 1);
    const maxRows = Math.max(stadiumsData.length, nationsData.length);

    let html = '<div style="display:flex;flex-direction:column;align-items:center;gap:3px;">';

    // v182: Full titles as headers
    html += '<div style="display:flex;width:100%;justify-content:center;gap:40px;margin-bottom:8px;font-size:0.7rem;color:#FFD700;font-weight:700;text-transform:uppercase;letter-spacing:1px;">';
    html += '<span>🏟️ Most Hosted Stadiums ←</span>';
    html += '<span>→ 🏳️ Most Hosted Nations</span>';
    html += '</div>';

    for (let i = 0; i < maxRows; i++) {
        const stadium = stadiumsData[i];
        const nation = nationsData[i];

        html += `<div class="split-row" style="height:${barH}px;">`;

        // Stadium bar (left side)
        if (stadium) {
            const proportion = stadium.matches / maxStadiumMatches;
            const sT = i / Math.max(stadiumsData.length - 1, 1);
            const sR = Math.round(80 + 120 * sT), sG = Math.round(160 - 80 * sT), sB = Math.round(200 - 100 * sT);
            const fillW = Math.round(leftHalf * proportion);
            html += `<div class="split-left" style="width:${leftHalf}px;height:100%;font-size:${fontSize};cursor:pointer;display:flex;align-items:center;border-radius:6px 0 0 6px;overflow:hidden;position:relative;" onclick="window.__lbClickStadium('${esc(stadium.stadium)}', '${tab}')" title="${esc(stadium.stadium)} — ${stadium.matches} matches">`;
            html += `<div style="position:absolute;left:0;top:0;width:${fillW}px;height:100%;background:rgba(${sR},${sG},${sB},0.85);"></div>`;
            html += `<div style="position:absolute;left:${fillW}px;top:0;width:${leftHalf - fillW}px;height:100%;background:rgba(${sR},${sG},${sB},0.25);"></div>`;
            // Match count (bold, far left)
            html += `<span style="position:relative;z-index:1;padding-left:8px;font-weight:700;color:#fff;white-space:nowrap;flex-shrink:0;">${stadium.matches}</span>`;
            // Stadium name (right-aligned near center)
            html += `<span style="position:relative;z-index:1;margin-left:auto;padding-right:8px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${esc(stadium.stadium)}</span>`;
            html += `</div>`;
        } else {
            html += `<div style="width:${leftHalf}px;height:100%;"></div>`;
        }

        html += `<div class="split-center" style="height:100%;"></div>`;

        // Nation bar (right side) — v182: tight flag-to-text spacing, bold numbers
        if (nation) {
            const proportion = nation.tournaments / maxNationTournaments;
            const nT = i / Math.max(nationsData.length - 1, 1);
            const nR = Math.round(255 - 30 * nT), nG = Math.round(180 - 80 * nT), nB = Math.round(40 + 20 * nT);
            const flag = flagImg(nation.nation) || '';
            const yearsStr = nation.years.join(', ');
            const fillW = Math.round(rightHalf * proportion);
            // v182: padding-left 24px (tight), flag at left:2px, text right after
            html += `<div class="split-right" style="width:${rightHalf}px;height:100%;font-size:${fontSize};cursor:pointer;white-space:nowrap;padding:0 6px 0 24px;position:relative;display:flex;align-items:center;overflow:hidden;border-radius:0 6px 6px 0;" onclick="window.__lbClickTeam('${esc(nation.nation)}', '${tab}')" title="${esc(nation.nation)} — ${nation.tournaments} tournaments (${yearsStr})">`;
            html += `<div style="position:absolute;left:0;top:0;width:${fillW}px;height:100%;background:rgba(${nR},${nG},${nB},0.85);"></div>`;
            html += `<div style="position:absolute;left:${fillW}px;top:0;width:${rightHalf - fillW}px;height:100%;background:rgba(${nR},${nG},${nB},0.25);"></div>`;
            // Flag (locked to left, tight spacing)
            html += `<span style="position:absolute;left:2px;top:50%;transform:translateY(-50%);z-index:1;display:flex;align-items:center;">${flag}</span>`;
            // Text: 3 · (1970, 1986, 2026) · 14 venues — bold numbers
            html += `<span style="position:relative;z-index:1;color:#fff;"><b>${nation.tournaments}</b> · (${yearsStr}) · <b>${nation.stadiums}</b> venues</span>`;
            html += `</div>`;
        } else {
            html += `<div style="width:${rightHalf}px;height:100%;"></div>`;
        }
    ...
    html += '</div>';
    html += '<div style="margin-top:12px;padding:8px;text-align:center;font-size:0.6rem;color:#888;border-top:1px solid rgba(255,255,255,0.1);">';
    html += 'Left: Stadiums by matches hosted · Right: Nations by tournaments hosted';
    html += '</div>';
    return html;
}
    ...
    if (type === 'player') {
        ...
        if (playerYear && parseInt(playerYear, 10) !== parseInt(histLookups.currentYear, 10)) {
            ...
            if (yearOpt) {
                ...
                updateFisheyeUI();
                filterAndDraw();
                try { await loadHistoricalData(playerYear); } catch (e) { void 0; }
                filterAndDraw();
...
function renderComparison() {
    ...
    // v165: Assign each entity a distinct color for the color-scale visualization.
    // These colors are used to tint stat cells based on relative performance.
    const entityColors = filled.map((_, i) => {
        // 4 distinct colors for up to 4 entities
        const palette = ['#4a8fe7', '#e74c3c', '#2ecc71', '#f39c12'];
    ...
    stats.forEach(s => {
        ...
        if (isNum && s.higher != null) {
            // v165: Color-scale for quantitative stats.
            // Find min and max across all entities (excluding the winner for contrast).
            const allVals = s.vals.filter(v => typeof v === 'number');
            ...
            s.vals.forEach((v, i) => {
                ...
                // v165: Compute color intensity based on relative position in range.
                // Winner gets full entity color; others get faded tint proportional to value.
                // For "lower is better" stats (higher:false), invert the scale.
                let bgColor = '';
                if (range > 0 && typeof v === 'number') {
                    const normalized = (v - minVal) / range; // 0 to 1
                    const intensity = s.higher ? normalized : (1 - normalized);
                    // Use the entity's color with opacity proportional to intensity
                    // Winner gets full opacity + bold; others get 0.15–0.5 opacity
                    const opacity = isWinner ? 0.55 : (0.15 + intensity * 0.35);
                    bgColor = `background:rgba(${hexToRgb(entityColors[i])},${opacity});`;
                }
                rowsHtml += `<div class="compare-stat-val ${isWinner ? 'winner' : ''}" style="text-align:center; ${bgColor} ${isWinner ? 'font-weight:800;' : ''}">${display}</div>`;
            });
        } else {
            // v165: String/non-quantitative stats — color code SIMILARITIES.
            // Entities with the same value get the same background color.
            // Different values get no background (white text on dark).
            const valGroups = {};  // value → [indices]
            s.vals.forEach((v, i) => {
                const key = String(v).toLowerCase().trim();
                if (!valGroups[key]) valGroups[key] = [];
                valGroups[key].push(i);
            });

            // Assign a color to each group (only if 2+ entities share the value)
            const groupColors = {};  // value → color
            let groupColorIdx = 0;
            const similarityPalette = ['rgba(78,205,196,0.35)', 'rgba(167,139,250,0.35)', 'rgba(241,196,15,0.35)', 'rgba(231,76,60,0.35)'];
            Object.entries(valGroups).forEach(([key, indices]) => {
                if (indices.length >= 2) {
                    groupColors[key] = similarityPalette[groupColorIdx % similarityPalette.length];
                    groupColorIdx++;
                }
            });
            ...
            s.vals.forEach((v, i) => {
                const key = String(v).toLowerCase().trim();
                const bgColor = groupColors[key] || '';
                rowsHtml += `<div class="compare-stat-val" style="text-align:center; ${bgColor ? 'background:' + bgColor + ';' : ''}">${v}</div>`;
            });
        }
        rowsHtml += '</div>';
    });
...
// v165: Helper — convert hex color to "r,g,b" string for rgba() usage.
...
// ==========================================
// v170: RIGHT-CLICK PARTICIPANT POPUP (one-line, scroll to cycle)
// ==========================================
// When a year is filtered, right-clicking anywhere shows a one-line popup
// to the right of the Replay button showing ONE team at a time.
// Scrolling over the popup cycles through teams (NO map movement).
// The highlighted team's lines glow with their "success" color; all other
// lines are DIMMED. Highlight persists across map move/zoom/drag because
// it's applied INSIDE the draw() function via window.__getHighlightedTeam().
// Only right-clicking again closes the popup.

(function() {
    let popupVisible = false;
    let participants = [];      // sorted array of { name, success, color, lat, lng }
    let currentIdx = 0;         // index into participants

    // v170: Override the global highlight accessors. draw() calls these on every
    // render to decide whether to apply highlight styles. This is the ONLY way
    // to keep the highlight across map move/zoom — D3's .join() recreates all
    // elements on every draw(), so any styles we apply afterwards get wiped.
    window.__getHighlightedTeam = function() {
        if (!popupVisible || !participants[currentIdx]) return null;
        return participants[currentIdx].name;
    };
    window.__getHighlightedTeamColor = function() {
        if (!popupVisible || !participants[currentIdx]) return null;
        return participants[currentIdx].color || '#FFD700';
    };

    // Right-click listener — ONLY way to open/close the popup
    document.addEventListener('contextmenu', function(e) {
        const yr = window.__getSelectedYear ? window.__getSelectedYear() : null;
        if (yr === null) return;
        e.preventDefault();
        ...
        if (popupVisible) {
            ...
            openParticipantPopup(yr);
        }
    }, true);

    // Scroll listener on the popup — cycle through teams, NO map zoom/pan
    document.addEventListener('wheel', function(e) {
        ...
        if (e.deltaY > 0) {
            ...
            currentIdx = (currentIdx - 1 + participants.length) % participants.length;
        }
        updatePopupDisplay();
        // v170: Re-trigger draw() so the highlight is applied to the new team.
        // The highlight accessors already return the new team, so filterAndDraw()
        // will render with the correct highlight.
        triggerRedraw();
    }, { capture: true, passive: false });

    function triggerRedraw() {
        // Call filterAndDraw() if available (re-renders with highlight)
        if (typeof filterAndDraw === 'function') {
            filterAndDraw();
    ...
    function openParticipantPopup(year) {
        if (typeof fifaWorldCupParticipants === 'undefined') return;

        participants = fifaWorldCupParticipants
            .filter(p => p.year === year && p.Success !== 'Host')
            .map(p => ({
                name: p.name,
                success: p.Success,
                // v174: Use special color for top finishers; for Group/Round 16,
                // look up the team's actual node color (which uses yearColorScale hue shift).
                color: getSuccessColor(p.Success) || getNodeColor(p.name, year) || '#fff',
                lat: p.lat,
                lng: p.lng
            }))
        ...
        popupVisible = true;
        updatePopupDisplay();
        triggerRedraw();  // render with highlight
    }

    function getSuccessColor(success) {
        // v174: Only top finishers and hosts have fixed special colors.
        // Group, Round 16, and Round 32 use the year color (vibrantColor) — but we
        // can't access yearColorScale here (it's in a different script block).
        // Instead, we look up the team's actual node color from allNodes, which
        // was set during processData() as specialColors[p.Success] || vibrantColor.
        // The caller passes the team name via participants[currentIdx], and we
        // look up the matching node in allNodes to get its actual color.
        const specialColors = {
            "Host": "#00FF00", "Host-Centenary": "#00FF00",
            "Champion": "#FFD700", "Final": "#C0C0C0",
            "Semi-Final": "#CD7F32", "Quarter-Final": "#FF6F92",
            "Round 32": "#FBCEFF"
        };
        return specialColors[success] || null;  // null = use node's actual color
    }

    // v176: Look up the team's actual node color from allNodes.
    // Host nations have TWO entries: one with Success="Host" (green) and one with
    // their actual tournament result (e.g., "Round 16"). We must find the NON-Host
    // entry to get the correct achievement color, NOT the green host color.
    function getNodeColor(teamName, year) {
        const g = window.__getD3Group ? window.__getD3Group() : null;
        if (g) {
            let foundColor = null;
            g.selectAll('circle').each(function(d) {
                if (d.name === teamName && d.year === year && foundColor === null) {
                    // v176: Skip the "Host" entry — we want the team's actual achievement color.
                    // Host nations have two nodes: one green (Host) and one with their result.
                    if (d.Success !== 'Host') {
                        foundColor = d.color;
                    }
                }
            ...
            // Fallback: if no non-Host entry found, use any matching node's color
            g.selectAll('circle').each(function(d) {
                if (d.name === teamName && d.year === year && foundColor === null) {
                    foundColor = d.color;
                }
        ...
        if (typeof allNodes !== 'undefined') {
            // v176: Find non-Host entry first
            let node = allNodes.find(n => n.name === teamName && n.year === year && n.Success !== 'Host');
            if (!node) node = allNodes.find(n => n.name === teamName && n.year === year);
    ...
    function updatePopupDisplay() {
        ...
        if (!p) return;
        const flag = flagImg(p.name) || '';
        popup.innerHTML = `${flag}<span class="pp-name">${esc(p.name)}</span><span class="pp-counter">${currentIdx + 1}/${participants.length}</span><span class="pp-hint">scroll</span>`;
    ...
    function closeParticipantPopup() {
        ...
        popup.innerHTML = '';
        popupVisible = false;
        // v170: Clear the highlight accessors and re-render to restore normal styles
        triggerRedraw();
    }

    // v176: Reload the popup with a new year's data WITHOUT closing it.
    // Rebuilds the participant list, resets to first participant, and re-highlights.
    // Called when the year changes while the popup is open.
    function reloadParticipantPopup(year) {
        if (!popupVisible) return;  // only reload if popup is open
        if (typeof fifaWorldCupParticipants === 'undefined') return;
        ...
        // Try to keep the same team selected if it exists in the new year
        const prevName = participants[currentIdx] ? participants[currentIdx].name : null;
        const newIdx = prevName ? participants.findIndex(p => p.name === prevName) : -1;
    ...
    // Expose for debugging and year-change reload
    window.__participantPopup = { open: openParticipantPopup, close: closeParticipantPopup, reload: reloadParticipantPopup };
})();
